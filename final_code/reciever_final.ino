...
const float accelFactor = 0.025;
const float brakeFactor = 0.08;
...
void turnRight(int s) {
  setDriver(FL_PWM, FL_DIR, s, true);   // FL backward
  setDriver(RL_PWM, RL_DIR, s, true);   // RL backward
  setDriver(FR_PWM, FR_DIR, s, true);   // FR forward
  setDriver(RR_PWM, RR_DIR, s, false);  // RR forward
}

// Tank Turn Right: Left side forward, Right side reverse
void turnLeft(int s) {
  setDriver(FL_PWM, FL_DIR, s, false);  // FL forward
  setDriver(RL_PWM, RL_DIR, s, false);  // RL forward
  setDriver(FR_PWM, FR_DIR, s, false);  // FR backward
  setDriver(RR_PWM, RR_DIR, s, true);   // RR backward
}
